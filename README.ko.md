...
- 앱 아이디어를 Expo MVP로 빠르게 정리하고 싶은 팀
- 프로젝트 생성부터 심사까지 반복 가능한 기준이 필요한 개발자
- Codex, Claude Code, 또는 `SKILL.md`를 읽는 에이전트를 쓰는 팀
- private 기본값 없이 공개 가능한 Expo 실무 문서가 필요한 프로젝트

## 포함된 것

- Expo 앱 생성, 마이그레이션, 기본 구조 설계
- 앱 아이디어를 MVP, 제품 청사진, 기능 로드맵, 스킬 실행 계획으로 정리하는 기획 스킬
- 바텀탭, 앱바, 바텀시트 모달, 설정 화면, 테마, 스플래시, 앱 아이콘을 위한 앱 셸 보일러플레이트
- Expo Router, 테마, 폰트, 아이콘, 시작 화면, OTA
- shimmer, skeleton, empty, error, offline, progress 로딩 패턴
- Expo Router 앱을 위한 컴포넌트 구조와 폴더 아키텍처
- Supabase, Appwrite, Firebase 백엔드 스킬
- Supabase/Appwrite cloud 또는 self-hosted 구성 문서
- Supabase, Appwrite, Firebase, Google, Apple, Kakao 계정 자동화와 브라우저 콘솔 fallback 실행 문서
- Kakao, Google, Apple 소셜 로그인 체크리스트
- 네이버 지도, 카카오 지도, 구글 지도, 애플 지도 선택 기준과 마커/클러스터 최적화
...
- development, staging, production flavor와 EAS/OTA/fastlane 릴리스 매트릭스
- EAS cloud build, local EAS build, OTA 선택 정책
- Expo Updates protocol 기반 custom OTA 서버 구현, manifest, asset hosting, rollout, rollback, code signing 가이드
...
2. 아이디어와 MVP 범위가 흐리면 `expo-idea-composer`로 제품 청사진을 먼저 만듭니다.
3. `expo-team-conventions`로 팀별 기본값을 적용합니다.
4. `expo-project-foundation`으로 앱 기반 구조를 만듭니다.
5. `expo-app-shell-boilerplate`로 공통 앱 셸을 만들고, `expo-router-navigation`, `expo-ui-system`으로 화면 구조와 UI 시스템을 다듬습니다.
6. `expo-startup-ota`, `expo-auth-secure-storage`로 시작 흐름과 인증을 붙입니다.
7. `expo-supabase-backend`, `expo-appwrite-backend`, `expo-firebase-backend` 중 백엔드를 고릅니다.
8. 미디어, 지도, 오프라인, 알림, 백그라운드, 성능 스킬을 추가합니다.
9. 스토어 출시 전 `expo-environment-flavors`, `expo-store-console-setup`, `expo-store-review-info`, `expo-android-jks-signing`, `expo-eas-build-strategy`, `expo-release-operator`, `expo-version-ota-governance`, `expo-fastlane-automation`, `expo-release-review`를 사용합니다.
...
필요한 것부터 복사하세요. 출시 단계에 가까워질수록 release/store 관련 문서를 추가하면 됩니다.
