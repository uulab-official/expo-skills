...
2. Inspect `docs/environment-flavors.md`, `docs/versioning-policy.md`, `docs/custom-ota-server.md`, `app.config.*`, `app.json`, `eas.json`, `package.json`, `store.config.*`, update scripts, and release docs.
...
- Use `expo-custom-ota-server` when implementing or auditing the custom server itself.
...
- If using a custom update server, `docs/custom-ota-server.md` records manifest endpoint, asset base URL, publish command, rollback command, monitoring, and code signing status.
...
- Document the custom update server URL and publish command in `docs/custom-ota-server.md`, `docs/update-server-policy.md`, and `docs/versioning-policy.md`.
